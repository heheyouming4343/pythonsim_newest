...
void Agent::ProcessLaneChangeRequest(Road *road, int LCdir, int LCmode,float moveLatDist)
{
    qDebug() << "[ProcessLaneChangeRequest]";
        ...
        if (LCdir == 0) {
        LCdir = DIRECTION_LABEL::RIGHT_CROSSING;
    } else if (LCdir == 1) {
        LCdir = DIRECTION_LABEL::LEFT_CROSSING;
    }
    qDebug() << "[DIRECTION_LABEL] LEFT =" << DIRECTION_LABEL::LEFT_CROSSING
             << " RIGHT =" << DIRECTION_LABEL::RIGHT_CROSSING;
    if( LCdir != DIRECTION_LABEL::LEFT_CROSSING && LCdir != DIRECTION_LABEL::RIGHT_CROSSING ){
        return;
    }

     int routeIndex = memory.routeIndex;
    int lcSupportIndex = memory.LCSupportRouteLaneIndex;

 
    for (int i = 0; i < road->odRoute[routeIndex]->LCSupportLaneLists.size(); ++i) {
        qDebug() << "LCSupport[" << i << "] LCDirect ="
                 << road->odRoute[routeIndex]->LCSupportLaneLists[i]->LCDirect;
        for (int j = 0; j < road->odRoute[routeIndex]->LCSupportLaneLists[i]->laneList.size(); ++j) {
            qDebug() << " - laneList[" << j << "] = "
                     << road->odRoute[routeIndex]->LCSupportLaneLists[i]->laneList[j];
        }
    }
    ...
    if (kk < 0 || kk >= road->odRoute.size()) {
    qDebug() << "Invalid memory.routeIndex: out of bounds!";
    return;
}

if (ll < 0 || ll >= road->odRoute[kk]->LCSupportLaneLists.size()) {
    qDebug() << "Invalid memory.LCSupportRouteLaneIndex: out of bounds!";
    return;
}
    ...
    const float tdevThreshold = 0.0f;  // 放宽判定阈值
...
//        }
        ...
        if( objPathInLCPath >= 0 ){
            if( LCdir == DIRECTION_LABEL::RIGHT_CROSSING && tdev > tdevThreshold  ){
                candidateList.append( i );
                tdevList.append( tdev );
            }
            else if( LCdir == DIRECTION_LABEL::LEFT_CROSSING && tdev < -tdevThreshold  ){
                candidateList.append( i );
                tdevList.append( tdev );
            }
        }
    ...
    if (candidateList.isEmpty()) {
        qDebug() << "No valid candidate lane found — skip lane change.";
        return;
    }
    ...
    //hehe
    if (LCTargetLane < 0 || LCTargetLane >= road->odRoute[kk]->LCSupportLaneLists[ll]->laneList.size()) {
    qDebug() << "Invalid LCTargetLane index: " << LCTargetLane;
    return;}
    ...
    if( LCmode == 0 ){
        ...
        if (memory.targetPathList.isEmpty()) {
        qDebug() << "[ERROR] laneChangeTargetPathList is empty!";
        return;
        }
